...
var Driver = function()
{
	...
	var path = "/homes/jjenkins/Desktop/data_sets/econTools-master/";
	var firms = []; //new ArrayList<Firm>();
	var econo;
	var utils = new EconUtils(path);
	var cusips = 100; // what is this?
		...
		init:																	function()
																					{
																						...
																						//setup elements
																						var f = new ReadFile(path).init(); //ReadFile
																						econo = f.getEconomy();
																						econo.createFirmTransitionObj(econo.cusipList);
																						var vals = doRoutine(); //Object[]
																						//define file paths
		...
		doRoutine:														function()
																					{
																						...
																						{
																							list1.push(checkNaNForBV(list1, econo.BeforeTree.get(cusips.get(i)), cusips.get(i)));
																							list2.push(checkNaNForBV(list2,  econo.DuringTree.get(cusips.get(i)), cusips.get(i)));
																							list3.push(checkNaNForBV(list3,  econo.AfterTree.get(cusips.get(i)), cusips.get(i)));			
																						}
